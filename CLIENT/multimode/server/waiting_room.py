...
class WaitingRoomQueue:
    ...
    def call_two(self) -> tuple[Player]|None:
...
class WaitingRoom:
    ...
    def __init__(self, count_start: int, observer) -> None:
        ...
        self.launcher = Thread(target= lambda: self.timer(count_start), daemon=True)
        self.launcher.start()
    ...
    def timer(self, interval: int) -> None:
        while True:
            sleep(interval)
            triggered = Thread(target=self.launch_with_two, daemon=True)
            triggered.start()
